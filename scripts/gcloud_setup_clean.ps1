...
try {
    ...
    if ($LASTEXITCODE -eq 0 -and $firestoreExists) {
...
foreach ($role in $deployerRoles) {
    ...
    gcloud projects add-iam-policy-binding $ProjectId `
        ...
        --quiet
    if ($LASTEXITCODE -ne 0) {
        Write-Host "Warning: Failed to grant $role to deployer SA" -ForegroundColor Yellow
    }
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0 -and $runtimeSAExists) {
...
foreach ($role in $runtimeRoles) {
    ...
    gcloud projects add-iam-policy-binding $ProjectId `
        ...
        --quiet
    if ($LASTEXITCODE -ne 0) {
        Write-Host "Warning: Failed to grant $role to runtime SA" -ForegroundColor Yellow
    }
}
...
try {
    ...
    if (-not (Test-Path $tempKeyPath)) {
        ...
        # Generate using Python with the project dependencies
        try {
            python -c "import nacl.signing; sk = nacl.signing.SigningKey.generate(); open('$tempKeyPath', 'wb').write(bytes(sk))"
            if ($LASTEXITCODE -ne 0) {
                throw "Python key generation failed"
            }
        } catch {
...
"@ | Out-File -FilePath $tempKeyPath -Encoding UTF8
            Write-Host "  Created placeholder key file. Please replace with real Ed25519 key." -ForegroundColor Yellow
        }
    }
...
Write-Host "  1. Run: .\scripts\deploy_cloudrun.ps1" -ForegroundColor White
